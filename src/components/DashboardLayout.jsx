...
import {
  Zap, LogOut, Home, Activity,
  UserCircle, UserCheck, Users, ChevronDown, Upload
} from 'lucide-react'

export default function DashboardLayout({ children, title, subtitle, tickerItems = [] }) {
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h1 className="font-display text-lg leading-tight">{title || 'SPUG Energy Portal'}</h1>
                {subtitle && <p className="text-teal-light text-xs">{subtitle}</p>}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {isSuperAdmin && (
              <div className="hidden sm:flex items-center gap-1">
                <NavLink icon={<UserCheck className="w-3.5 h-3.5" />} label="Approvals" active={location.pathname === '/admin/approvals'} onClick={() => navigate('/admin/approvals')} />
                <NavLink icon={<Users className="w-3.5 h-3.5" />}     label="Users"     active={location.pathname === '/admin/users'}     onClick={() => navigate('/admin/users')} />
                <NavLink icon={<Upload className="w-3.5 h-3.5" />}    label="Upload"    active={location.pathname === '/admin/upload'}    onClick={() => navigate('/admin/upload')} />
              </div>
            )}
            ...
            <div className="relative" ref={menuRef}>
              ...
              >
                ...
                {(isSuperAdmin || isAdmin) && (
                  <span className={`hidden sm:block text-xs px-1.5 py-0.5 rounded-full capitalize ${roleBadgeColor[profile?.role] || 'bg-white/20 text-white/90'}`}>
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-gray-100 py-1.5 z-50">
                  <p className="text-xs text-gray-400 px-3 py-1.5 font-medium truncate">{user?.email}</p>
                  ...
                  {isSuperAdmin && (
                    <>
                      ...
                      <button onClick={() => { navigate('/admin/approvals'); setMenuOpen(false) }}
                        className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition">
                        <UserCheck className="w-4 h-4 text-gray-400" /> User Approvals
                      </button>
                      <button onClick={() => { navigate('/admin/users'); setMenuOpen(false) }}
                        className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition">
                        <Users className="w-4 h-4 text-gray-400" /> Manage Users
                      </button>
                      <button onClick={() => { navigate('/admin/upload'); setMenuOpen(false) }}
                        className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition">
