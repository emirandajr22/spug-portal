...
import {
  Zap, LogOut, ChevronRight, Activity, Building2,
  UserCircle, UserCheck, Users, ChevronDown, Upload
} from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-offwhite flex flex-col">
      ...
      <header className="bg-moss text-white shadow-lg">
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display text-xl leading-tight">SPUG Energy Portal</h1>
              <p className="text-teal-light text-xs">Small Power Utilities Group</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {isSuperAdmin && (
              <div className="hidden sm:flex items-center gap-1">
                <button onClick={() => navigate('/admin/approvals')} className="flex items-center gap-1.5 text-sm text-white/60 hover:text-white border border-transparent hover:border-white/20 rounded-lg px-3 py-1.5 transition-all">
                  <UserCheck className="w-3.5 h-3.5" /> Approvals
                </button>
                <button onClick={() => navigate('/admin/users')} className="flex items-center gap-1.5 text-sm text-white/60 hover:text-white border border-transparent hover:border-white/20 rounded-lg px-3 py-1.5 transition-all">
                  <Users className="w-3.5 h-3.5" /> Users
                </button>
                <button onClick={() => navigate('/admin/upload')} className="flex items-center gap-1.5 text-sm text-white/60 hover:text-white border border-transparent hover:border-white/20 rounded-lg px-3 py-1.5 transition-all">
            ...
            <div className="relative" ref={menuRef}>
              ...
              >
                ...
                <span className="hidden sm:block max-w-[140px] truncate">{username}</span>
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-1.5 z-50">
                  ...
                  {isSuperAdmin && (
                    <>
                      <div className="border-t border-gray-100 my-1" />
                      <p className="text-xs text-gray-400 px-3 py-1 font-medium">Super Admin</p>
                      <button onClick={() => { navigate('/admin/approvals'); setMenuOpen(false) }} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition">
                        <UserCheck className="w-4 h-4 text-gray-400" /> User Approvals
                      </button>
                      <button onClick={() => { navigate('/admin/users'); setMenuOpen(false) }} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition">
                        <Users className="w-4 h-4 text-gray-400" /> Manage Users
                      </button>
                      <button onClick={() => { navigate('/admin/upload'); setMenuOpen(false) }} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition">
                  ...
                  {isAdmin && (
                    <>
                      <div className="border-t border-gray-100 my-1" />
                      <p className="text-xs text-gray-400 px-3 py-1 font-medium">Admin</p>
                      <button onClick={() => { navigate('/admin/upload'); setMenuOpen(false) }} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 transition">
                        <Upload className="w-4 h-4 text-gray-400" /> Upload Data
                      </button>
                    </>
        ...
        <div className="scroll-animate text-moss text-xs font-mono px-8">
          {[1,2].map(n => (
            <span key={n}>
              ...
              SPUG Energy Dashboard &nbsp;·&nbsp;
              DPI — Delta P, Inc. &nbsp;·&nbsp;
              INPC — Isla Norte Power Corporation &nbsp;·&nbsp;
              CIPC — Busuanga &nbsp;·&nbsp; CIPC — Coron &nbsp;·&nbsp; CIPC — EPSA &nbsp;·&nbsp;
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-6 py-12">
        <div className="mb-10">
          <h2 className="font-display text-3xl text-moss mb-2">Energy Dashboards</h2>
            ...
            >
              ...
              >
                <svg className="absolute inset-0 w-full h-full opacity-10"
                  ...
                  <defs>
                    <pattern id={`dots-${company.id}`} width="20" height="20"
                ...
                <span className="absolute top-4 right-4 bg-white/20 text-white/90
              ...
              <div className="p-5">
                ...
                <p className="text-moss/55 text-sm mb-4">{company.description}</p>
                ...
                {company.subDashboards ? (
                  <div className="space-y-2">
                    {company.subDashboards.map(sub => (
                      ...
                      >
                        {sub.label}
                        <ChevronRight className="w-4 h-4 text-teal opacity-0
      ...
      <footer className="text-center text-moss/40 text-xs py-6 border-t border-sky/30">
        Created by: VEC-Settlement &nbsp;·&nbsp; © {new Date().getFullYear()} All rights reserved.
      </footer>
    </div>
  )
}
